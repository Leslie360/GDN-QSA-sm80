...
- **Kernel C (score)**: tiled like a small GEMM — `SCORE_CQ=16` query rows share
  one staged 32-block key tile (each block key is read 16× from shared memory),
  `512` threads, one thread per (query, block) cell, scalar float4 dots.  The
  cross-query block-key reuse is what makes long sequences fast: halving
  per-score shared-memory traffic vs `SCORE_CQ=8`.
- **Kernel D (topk)**: per query, histogram the top 8 bits of a packed
  ...
  candidates above it → bitonic-sort only the `~block_topk` winners descending.
  The packed 64-bit key gives the reference's exact order (higher score first;
  equal score → lower block index first).  CPU-validated in
  `tests/radix_topk_proto.py` (5000/5000 vs a full sort).
